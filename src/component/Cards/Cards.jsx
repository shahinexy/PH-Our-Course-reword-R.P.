const Cards = ({ datas, handleAddToCart }) => {
  return datas.map((data) => (
    <div key={data.id}>
      <div className="p-3 text-center space-y-3 bg-cyan-100 border-[1px] border-cyan-300 shadow-lg rounded-xl h-full">
        ...
        <button onClick={()=> handleAddToCart(data)} className="bg-cyan-600 hover:bg-cyan-700 py-2 px-5 rounded-lg border-[1px] border-cyan-800 font-semibold">
          Add to Cart
        </button>
      </div>
    </div>
